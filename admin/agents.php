...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      $select_images = $conn->prepare("SELECT * FROM `property` WHERE agent_id = ?");
      ...
      $delete_listings = $conn->prepare("DELETE FROM `property` WHERE agent_id = ?");
...
<section class="grid">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_agents = $conn->prepare("SELECT * FROM `agents` WHERE name LIKE '%{$search_box}%' OR number LIKE '%{$search_box}%' OR email LIKE '%{$search_box}%'");
      ...
      if($select_agents->rowCount() > 0){
         while($fetch_agents = $select_agents->fetch(PDO::FETCH_ASSOC)){
            ...
            $count_property = $conn->prepare("SELECT * FROM `property` WHERE agent_id = ?");
            $count_property->execute([$fetch_agents['id']]);
            $total_properties = $count_property->rowCount();
   ?>
   <div class="box">
      <p>Name : <span><?= $fetch_agents['name']; ?></span></p>
      <p>Number : <a href="tel:<?= $fetch_agents['number']; ?>"><?= $fetch_agents['number']; ?></a></p>
      <p>Email : <a href="mailto:<?= $fetch_agents['email']; ?>"><?= $fetch_agents['email']; ?></a></p>
   ...
   <?php
      ...
      echo '<p class="empty">no agents accounts added yet!</p>';
   }
   ?>
